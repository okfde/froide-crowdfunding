...
from froide_payment.models import CHECKOUT_PAYMENT_CHOICES_DICT, Order
...
def get_editable_crowdfunding(crowdfundings):
    editable = [c for c in crowdfundings if c.status in Crowdfunding.REVIEW_STATUS]
    if not editable:
        return None
    if len(editable) > 1:
        return None
...
class CrowdfundingRequestStartForm(forms.ModelForm):
    title = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"placeholder": _("Title of campaign"), "class": "form-control"}
        ),
    )
    ...
    description = forms.CharField(
        ...
        help_text=_("Describe why getting the information is important " "to you."),
        widget=forms.Textarea(attrs={"class": "form-control"}),
    )

    public_interest = forms.CharField(
        label=_("Why is this important to the public?"),
        help_text=_("Describe how this case affects the public."),
        widget=forms.Textarea(attrs={"class": "form-control"}),
    )
    ...
    amount_requested = forms.DecimalField(
        ...
        widget=PriceInput,
        help_text=_(
            "Please specify the amount you need to raise "
    ...
    terms = forms.BooleanField(
        ...
        error_messages={"required": _("You need to accept our crowdfunding terms.")},
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.crowdfundings = Crowdfunding.objects.filter(request=self.foirequest)
