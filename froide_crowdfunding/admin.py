...
from .models import Contribution, Crowdfunding
    ...
    def save_model(self, request, obj, form, change):
        if "status" in form.changed_data:
            ...
            if obj.status == "denied":
                send_template_email(
                    user=obj.user,
                    subject=_("💸 Your crowdfunding project needs review"),
                ...
                send_template_email(
                    user=obj.user,
                    subject=_("💸 Your crowdfunding project has been activated"),
...
class ContributionAdmin(admin.ModelAdmin):
    ...
    list_filter = ("status", "public", ("crowdfunding", ForeignKeyFilter))
    search_fields = ("crowdfunding__title",)
    date_hierarchy = "timestamp"

    def get_queryset(self, request):
        qs = super().get_queryset(request)
        qs = qs.prefetch_related("crowdfunding", "user", "order")
        return qs
