...
class Crowdfunding(models.Model):
    ...
    FINAL_STATUS = (CrowdfundingStatus.FAILED, CrowdfundingStatus.SUCCESSFUL)
    REVIEW_STATUS = (CrowdfundingStatus.DENIED,)
    PUBLIC_STATUS = (CrowdfundingStatus.RUNNING,) + FINAL_STATUS
    status = models.CharField(
        max_length=25, choices=CrowdfundingStatus.choices, default="needs_approval"
    )
    ...
    date_requested = models.DateTimeField(default=timezone.now, null=True, blank=True)
    ...
    amount_tentative = models.DecimalField(
        decimal_places=2, max_digits=10, default=Decimal(0.0)
    )
    ...
    class Meta:
        ...
        permissions = (("can_crowdfund", _("Can use crowdfunding")),)
    ...
    def get_absolute_url(self):
        return reverse("crowdfunding:crowdfunding-detail", kwargs={"slug": self.slug})
    ...
    def get_start_contribution_url(self):
        return reverse(
            "crowdfunding:crowdfunding-start_contribution", kwargs={"pk": self.pk}
        )

    def update_amount(self, save=True):
        contributions = (
            self.contribution_set.all()
...
class Contribution(models.Model):
    ...
    order = models.OneToOneField(
        "froide_payment.Order", null=True, blank=True, on_delete=models.SET_NULL
    )
    status = models.CharField(
    ...
    def __str__(self):
        return "%s from %s (%s)" % (self.amount, self.get_email(), self.pk)
...
if CMSPlugin:
    ...
    class CrowdfundingFormCMSPlugin(CMSPlugin):
        crowdfunding = models.ForeignKey(
            Crowdfunding, related_name="+", on_delete=models.CASCADE
        )
